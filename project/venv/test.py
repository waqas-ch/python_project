...
class attendence_register:
    def __init__(self, root,at_id):
         ...
         self.fetch_button = Button(self.rightMayFrame_2, text="Enter Records->", padx=2, pady=2, bd=2, fg="black",
                                    font=('arial', 10, 'bold'), width=11, height=1,command=self.enter_records).grid( row=0,column=1)
         self.fetch_button = Button(self.rightMayFrame_2, text="Fetch Next->", padx=2, pady=2, bd=2, fg="black",
         font=('arial', 10, 'bold'), width=11, height=1, command=self.fetch_next).place(x=120, y=120)
    ...
    def add_student_to_register(self,name, number,  i):
       ...
       # Course No.
       self.lblCourse_Code = Label(self.leftMayFrame_2, font=('arial', 10, 'bold'), text="100S", padx=2,
                              pady=2, bd=2, fg="Black", width=12)
    ...
    def startfetch(self):
       ...
       sqlite_select_query = """SELECT * FROM Student  JOIN Enrollment on(Email=en_student) Where en_course=1"""
